...
> 固定の質問集に2つの構成を通して正解ラベルと突き合わせる **offline evaluation** で、
> 同じ質問に両方を通す **対応のある比較（paired comparison）**。
...
この考え方は **ViDoRe**（ColPali と同時に公開された文書画像検索のベンチマーク）が
「視覚的に情報が詰まったページ」と「テキストで足りるページ」を分けて評価しているのと同じ。
混ぜて平均すると差が消える。
