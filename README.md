...
| **5-3**（次段）原本画像を根拠にした回答生成 | **DocVQA / VisualMRC / JDocQA** | 文書画像に対する QA の正答率（日本語は JDocQA） |
| **5-4**（保留）チャート読解支援 | **ChartQA / CharXiv** | チャート画像からの読み取り精度。5-4 の「予測の前に、そもそも読めているか」を測る土台 |
