...
def llm_rerank(question: str, candidates: list[dict], top_n: int = TOP_K) -> list[dict]:
    """(TODO / 次の実験) 候補をLLMに渡して関連度で並べ替え、上位top_nを返す。

    実装の骨子:
      - 候補チャンクに番号を振ってプロンプトに列挙
      - 「質問に最も関連する順に番号を返せ」とClaudeに指示
      - 返ってきた順で並べ替えて top_n を返す
    まずは RRF 版で精度を測り、リランク有り/無しを評価で比較するのが目的。
    """
    raise NotImplementedError  # TODO: まずはこの下の hybrid_search(rerank無し)で動かす


def hybrid_search(question: str, top_n: int = TOP_K) -> list[dict]:
    """ベクトル検索 + 字面検索 を RRF で融合し、上位top_nを返す。"""
