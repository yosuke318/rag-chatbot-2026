...
測るもの（検索側）:
  ...
  ★何を「正解」と数えるかは設問ごとに決まる★（_matches 参照）
    ...
    いる限り効果が数値に出ない（過去の比較評価が ±0.000 だった理由）。
...
図表（画像）の検索を比較評価する:
  ...
  ★準拠ベンチマーク: ViDoRe（Visual Document Retrieval Benchmark, 2024）★
  「テキスト化してから検索」対「ページ画像を直接検索」を比べるための評価設計で、
  ここの案A(caption)対案B(multimodal)と構図が一致する。ViDoRe は視覚的に情報が
  詰まったページとテキストで足りるページを分けて集計する ─ この2条件も同じ理由で、
  混ぜて平均すると差が消える。データセット自体は使わず評価設計だけを借りている。
...
# 評価専用コーパスの質問集。デモ用の fixture と分けてあるのは、
...
def _matches(
    ...
    ★文書名だけではチャンク単位の改良を測れない★
      ...
      この粒度では原理的に差が出ない（過去の比較評価が ±0.000 だった理由）。
    ...
    ★文書名だけでは図表の検索も測れない★
      ...
      それでは図表の索引方式(案A/案B)を変えても数字が動かず、比較にならない。
